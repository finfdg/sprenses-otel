...
| **Revizyon** | **R1 · 2026-07-25 04:35** — 3 bulgu kapatıldı (FIN-001 Kritik · DB-001 · JOBS döviz cron'u).<br>**R2 · 2026-07-25 05:15** — ARCH-001 kapatıldı **ve bu raporun bir bulgusundaki yanlış canlı-sapma iddiası düzeltildi** (bkz. R2 Kapanış Kaydı).<br>**R3 · 2026-07-25 05:35** — DR ailesi: DR-001 büyük ölçüde kapandı (uploads yedekte + tatbikat), DR-003 kapatıldı (alarm kanalı canlı), DR-002 AWS provizyonuna bağlı açık.<br>**R5 · 2026-07-25 10:30** — İLK GERÇEK RESTORE TATBİKATI koşuldu (`docs/denetim/2026-07-25-restore-tatbikati.md`): başarılı, ama **R3'teki izin sertleştirmesinin felaket kurtarmayı sessizce kırdığını yakaladı**. RPO/RTO ilk kez tanımlandı.<br>**R4 · 2026-07-25 10:05** — CICD-010'un **ön koşulları** açıldı: sıfırdan bootstrap'ı engelleyen 3 kusur (migration FK sırası · seed'in 14 modül geride olması · 7 bayat modül yaması) düzeltildi. Taze DB'de 1927 test yeşil. Bulgu kapanmadı — Actions'ı açmak hâlâ depo sahibinde. Bulgu metinleri **silinmedi**; durumlar güncellendi, hatalı iddia üstü çizilerek gerekçesiyle bırakıldı — denetim anının fotoğrafı ve hatanın izi birlikte korunur.<br>**R7 · 2026-07-26** — **CICD-010 kısmen kapandı ve altından ikinci bir engel çıktı.** GitHub Actions depo düzeyinde etkinleştirildi; `ci.yml` 2026-06-02'den beri ilk kez **fiilen tetiklendi** (`total_count` 0→1). Ancak işler başlamıyor: hesap **faturalandırma kilidinde**. Denetimin "15 dakikalık depo ayarı" teşhisi eksikti — gerçek engel bir katman yukarıdaydı. Yeni alt-bulgu **CICD-010b** açıldı; asıl bulgu ◐ (kısmi) işaretlendi, kapalı sayılmadı. Ayrıca **takvime bağlı kırılgan bir test** yakalandı ve düzeltildi (`test_payment_deferral.py::test_deferred_item_flagged`): runway penceresi ay-içi olduğu hâlde test bugünden +6 gün öteliyordu → ayın son 6 gününde **her ay** kırmızı. Kardeş testlerdeki ay-sınırı koruması eksikti; eklendi. CI canlıya alınacağı için bu sınıf önemli — takvim yüzünden kırmızı olan bir kapı, göz ardı edilmeyi öğretir. |
...
| 11 | CI/CD | 4 | **3** | 7 | ~~**Actions repo düzeyinde KAPALI**~~ → **R7: açıldı, koşu tetikleniyor** ama işler **hesap billing kilidi** yüzünden başlamıyor (yeni alt-bulgu **CICD-010b**) — v3'ün varsaydığı gate hiç var olmamış; ~~kalite kapısı eksik (svelte-check + kapsam eşiği gate'te değil)~~ → **CICD-013 KAPANDI**: svelte-check bloklayıcı (QUAL-001) + kapsam eşiği regresyon testiyle korundu (`test_ci_cicd013_quality_gate.py`); Actions açılınca canlı koşar (CICD-010'a bağımlı) |
...
|---|---|---|:--:|:--:|
| **CICD-010**<br>(=TSTP-001<br>=DEBT-001) | **CI 2026-06-02'den beri hiç çalışmadı** — GitHub Actions depo düzeyinde KAPALI; 451 commit test edilmeden master'a gitti. 1.917 testlik takım ve `--cov-fail-under=60` eşiği fiilen dekoratif. **→ ◐ KISMİ KAPANIŞ R7 (2026-07-26):** depo ayarı düzeltildi — `actions/permissions` artık `{"enabled":true,"allowed_actions":"all"}` ve `ci.yml` her push'ta **gerçekten tetikleniyor** (tarihte ilk koşu: `30187332124`, `total_count` 0→1). Regresyon bekçisi eklendi (`backend/tests/test_ci_cicd010_actions_enabled.py`, 4 test; Actions kapatılınca / push tetikleyicisi kaldırılınca / bir iş `if: false` ile susturulunca kırmızıya döndüğü fiilen doğrulandı). **Ama tam kapanmadı — denetimde görülmemiş İKİNCİ engel çıktı:** işler başlamıyor, GitHub `"The job was not started because your account is locked due to a billing issue."` döndürüyor. Bu **hesap düzeyinde** kilit (depo ayarı değil); depo public → Actions dakikaları ücretsiz olmasına rağmen tüm işleri engelliyor. Yeni alt-kalem: **CICD-010b**. | ✔ *ana denetçi:* `actions/permissions` → `{"enabled":false}`; `ci.yml/runs` → `total_count: 0`<br>✔ *R7:* `enabled:true` ölçüldü; koşu tetiklendi; 3 işin üçünde de billing annotation'ı (yeniden koşuda da aynı) | **S** | RISK_DUSUR → Yüksek |
| **CICD-010b**<br>*(yeni — R7)* | **GitHub hesabı faturalandırma kilidinde → Actions işleri hiç başlamıyor.** CICD-010'un düzeltilmesi sırasında ortaya çıktı: depo ayarı açıldıktan sonra koşu tetiklenir ama üç işin üçü de 1-3 sn'de `failure` olur ve tek annotation billing kilididir. CI fiilen hâlâ koruyucu değil. Denetim "Actions'ı aç, 15 dk" derken bu katmanı bilmiyordu — bulgunun maliyeti depo ayarı değil, **hesap faturalandırması**. | ✔ *R7:* `gh run view 30187332124` → 3 iş, hepsinde *"The job was not started because your account is locked due to a billing issue."*; `gh run rerun` sonrası aynı | **S** | ⬜ Açık — *yalnız hesap sahibi çözebilir (GitHub → Settings → Billing)* |
...
| 2 | **CICD-010** | GitHub Actions'ı **etkinleştir** + boş commit ile ilk koşuyu yeşile al | ~~15 dk~~ **15 dk** (ön koşullar R4'te açıldı — öncesinde CI ilk adımda ölüyordu) | 1.927 test ilk kez koruyucu hâle gelir | ◐ **R7** — Actions açıldı, koşu tetikleniyor; işler **billing kilidi** yüzünden başlamıyor → CICD-010b |
| 2b | **CICD-010b** | GitHub hesabının **faturalandırma kilidini** çöz (Settings → Billing) | **?** (hesap sahibi) | Tetiklenen koşular fiilen çalışır; 1.927 test ilk kez koruyucu olur | ⬜ Açık — *yalnız hesap sahibi* |
...
| **CICD-010** | `actions/permissions` → `enabled:true` **ve** en son `ci.yml` koşusunun conclusion'ı `success` | ◐ **R7** — 1. bacak ✔ (`enabled:true`, koşu tetikleniyor, `total_count` 0→1, 4 regresyon testi yeşil/geri-alınca kırmızı) · 2. bacak ✗ (**CICD-010b**: hesap billing kilidi işleri başlatmıyor) |
| **CICD-010b** | `gh run list --workflow=ci.yml` → en son koşuda billing annotation'ı **yok** ve conclusion `success` | ⬜ Açık — *hesap sahibi GitHub Billing kilidini çözmeli; kod tarafında yapılacak iş yok* |
