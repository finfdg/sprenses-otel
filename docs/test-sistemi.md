...
*Bileşenler:*
- `src/lib/components/ui/MoneyInput.test.ts` — formatTR/parseTR/formatLiveTR/round-trip + imleç/highlight (33 test)
...
- `src/lib/components/ui/Pagination.test.ts` — getPageNumbers (windowed), sayfa boyutu (16 test)
- `src/lib/components/ui/FileDropzone.test.ts` — drag-drop, MIME/boyut doğrulama, çoklu dosya (14 test)
- `src/lib/components/ui/SortableHeader.test.ts` — sıralama yönü/ikon (11 test)
- `src/lib/components/ui/EmptyState.test.ts` — ikon/başlık/açıklama/CTA (9 test)
- `src/lib/components/ui/StatusBadge.test.ts` — semantik durum renkleri (8 test)
- `src/lib/components/ui/Breadcrumb.test.ts` — kırılım üretimi (6 test)
- `src/lib/components/ui/TableSkeleton.test.ts` — satır/kolon iskeleti (6 test)
- `src/lib/components/ui/FormSkeleton.test.ts` — form iskeleti (5 test)
- `src/lib/components/ui/BulkActionsBar.test.ts` — toplu seçim/aksiyon barı (5 test)
- `src/lib/components/ui/PdfPreviewModal.test.ts` — iOS Safari uyumlu PDF önizleme modalı (açma/kapama, blob URL yaşam döngüsü, Esc, backdrop) (6 test)
