...
| `app/routers/common/scheduled_factory.py` | Generic CRUD router fabrikası |
...
### Etkilenen Dosyalar
- `backend/app/routers/common/scheduled_factory.py` — POST endpoint'inde pasif kayıt oluşturma
...
## Maaş Tahmini ↔ Sedna Bordro Senkronu (2026-07-18)

`hr.salary` planlı girişleri elle tahminle girilir; muhasebe bordroyu Sedna'ya işleyince
(335 Personele Borçlar hesabı) tahmin **otomatik gerçek tahakkuka güncellenir**:

- **Kaynak:** `sedna_client.fetch_personnel_payroll()` — 335 hesabının aylık **tahakkuk**
  (alacak) toplamı. Tahakkuk o ayın gerçek personel maliyetidir (maaş + kıdem + huzur hakkı
  dahil); ödeme (borç) toplamı kullanılmaz — ödeme kanıtı banka eşleştirmesinden gelir.
- **Kural (`services/salary_sync_service.py`):** yalnız **ödenmemiş + ayı bitmiş** dönemler
  güncellenir. Gelecek ayların mevsimsel elle tahminlerine dokunulmaz; ödenmiş dönem banka
  kanıtıyla kesinleşmiştir. Tahakkuk mevcut tahminin **%40'ının altındaysa** bordro henüz
  işlenmemiş sayılır (ay ortası 335 alacağı yalnız avans/icra kalemleri) → atlanır.
- **Tetikleme:** Sedna senkron adımı `salary_sync` (izin `hr.salary` use; Topbar "Sedna"
  butonu + `sprenses-sedna-sync.timer` 2 saatte bir). Güncellemeler audit'e yazılır
  (`salary_sedna_sync`).
- **Ödeme bacağı:** maaş/SGK/stopaj girişleri banka hareketleriyle otomatik eşleştirilir
  (çift sayım engeli + tutarın gerçeğe çekilmesi) — detay `docs/modules/nakit-akim.md`
  "Personel Birleştirmesi" bölümü.

Test: `tests/test_personel_birlestirme.py::TestSalarySedaSync`.
