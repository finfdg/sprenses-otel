...
| `currency` | varchar(3) | Para birimi (TRY, USD, EUR, GBP — GBP 2026-08-14'te eklendi, ilk hesap: Halkbank 55100175 — ilk kayıtta 2A000897 girilmişti, 2026-08-31'de banka ekranıyla düzeltildi; EUR çevrimi GBP/EUR çaprazıyla, bkz. finance/CLAUDE.md "GBP Çapraz Kura Alındı") |
...
**açık `db.delete(tx)`** — `bank_transactions.statement_id` FK'sı ON DELETE **SET NULL**
olduğundan cascade YOK; ilk sürüm cascade varsayıp işlemleri orphan bırakıyordu, Faz 3
regresyon testi yakaladı, 2026-07-12 düzeltildi) · `delete_bank_transaction` (tekil;
eşleşmişse `ValueError` → 400) · `delete_account_with_cleanup` (hesap; tüm işlemler
release + invalidate — hesap FK'sı CASCADE). **Router + onay executor ORTAK** — executor
`_handle_finance_banks` aynı servis fonksiyonlarını çağırır (`payload["op"]` ile ayrışır;
op yoksa hesap CRUD). Bu yüzden `finance.banks` simple-crud executor fabrikasından
ÇIKARILDI (açık handler).

**Test:** `tests/test_faz3_integrity.py` (31 test) — release matrisi (çek/kredi tekil +
N-1 grup/avans/cari çifti/KK), silme servisleri + DELETE endpoint'leri (sayaçlar, 400/404,
viewer 403, onay akışı 202→onay→executor regresyonu), upload başlık doğrulaması (#22b),
aging + forecast-accuracy raporları, bakiye-zinciri kontrolü (#22a).
