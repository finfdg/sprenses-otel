# Maliyet Kontrol (Operasyonel KPI)

## Genel Bilgi
- **Modül:** **Maliyet Kontrol** = `stok.maliyet` (operasyonel KPI füzyonu)
- **Frontend:** `/dashboard/stok/maliyet`
- **Backend:** `/api/stok/operational-kpi`, `/price-variance` (`app/routers/stock.py`)
- **Kaynak:** Sedna stok (TRY maliyet) + rezervasyon (geceleme/doluluk; **SednaPrenses önbürodan canlı senkron** — `docs/modules/otel-rezervasyon.md`) + finans modülleri — **füzyon**
...
Bu yüzden Maliyet Kontrol sayfasında eski "PMS bekleniyor" placeholder'ı **all-inclusive bilgi notuyla**
değiştirildi.
...
medyan/anomali ayrımı + izin). Canlı (7 Haz 2026, senkron sonrası): doluluk cari yıl ~%38.8 ·
ADR €236 · RevPAR €91.6 · 104.716 geceleme.
