...
| GET | `/accounting/mutabakat/items/pdf` | view | **Uyuşmazlık listesi PDF raporu (2026-07-22)** — `/items` ile AYNI filtre/sıralama parametreleri (`_apply_item_filters` ortak; sayfalama YOK, `_PDF_MAX_ROWS=1500` tavanı — aşım raporda amber uyarıyla açıkça yazılır). Yatay A4, `register_turkish_fonts()` (₺ dahil), tutar kırmızı/yeşil, kapalı kayıtta çözüm etiketi. `heavy_limiter` + salt-okuma GET → onaydan muaf. Frontend: filtre barındaki "PDF" butonu → `api.fetchRaw` → **`PdfPreviewModal`** (iPad Safari blob kuralı) |
| POST | `/accounting/mutabakat/run` | use | Mutabakat taramasını elle tetikle. Banka↔Sedna ana taraması aktif takvim yılını kapsar; ±3 gün eşleştirme için veri yılbaşından 3 gün önce çekilir, tampon bulgular ekranda gösterilmez. `window_days` (7–365, varsayılan 45) bakiye-zinciri gibi ikincil kontrollere uygulanır. **Onaydan MUAF** (sınıflandırma — veri mutasyonu değil) + audit + WS broadcast. Tünel kapalı → **503**. **Faz C özet alanları:** `locked_period_new` (kilit-öncesi tarihli yeni uyuşmazlık sayısı) + `negative_balances` (ters-bakiye listesi) + cari bakiye taraması **best-effort** koşulur → `vendors_scanned`/`balance_diffs`/`vendor_auto_closed` (başarısızsa `vendor_error` — banka taraması sonucu korunur). **Faz 3 (2026-07-12):** `balance_chain_breaks` (bakiye-zinciri kırılma listesi) |
...
| Aktif yıl + `DATE_WINDOW_DAYS` tamponu | Yılbaşı − 3 gün | Banka↔Sedna ana taraması aktif takvim yılının tamamını kapsar; yıl sınırındaki tarih kaymalı eşleşmeler için veri 3 gün erken alınır. Tampon dönem bulguları raporlanmaz. |
...
- **Karşı-hesap köprüsü (2026-07-23; genişletme 2026-08-05):** `_match_account` eşleşen
  çiftleri `match_groups_out` parametresiyle dışa verir; koşu sonunda
  `services/sedna_tag_bridge.apply_sedna_tag_bridge` ETİKETSİZ banka hareketlerini fişin
  karşı-hesap prefix'inden kategorize eder (335/196→Personel, 320→Cari, 360/361/368/369→
  Vergi/SGK, 300/303→Kredi/Leasing, 340→Acenta, 331→Temettü, 103→Çek Ödemesi,
  102↔102→Virman/Döviz Satışı; **2026-08-05 eklenenler:** 100→Kasa, 101→Çek Tahsilatı,
  159→Verilen Avanslar, 602→Diğer Gelirler, 780→Finansman Gideri; haritasız prefix —
  ör. 770 — atlanır). **120 (ALICILAR) alt-kırılımlıdır** (tek prefix'e indirgenemez —
  canlıda aynı 120 altında acente, kiracı, münferit misafir, telekom/ATM alıcıları var):
  `120.02`→Kira Geliri, `120.03`→Konaklama Tahsilatı; kalan 120 kodu `agency_code_map`'te
  varsa VE hesap adı `auto_tagger._AGENCY_NAME_BLOCK`'a takılmıyorsa→Acenta, aksi→Cari.
  Best-effort — köprü hatası mutabakat sonucunu düşürmez; özet `sedna_tagged` alanı taşır
  (senkron modalında "N karşı-hesaptan etiketlendi").
