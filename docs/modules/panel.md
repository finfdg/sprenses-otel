...
| Bileşen | `frontend/src/lib/components/dashboard/RunwayChart.svelte` — Nakit Koruma / runway projeksiyonu (ödeme erteleme what-if) |
...
3. **Nakit Akım** (`CashFlowTAccount`) — `GET /finance/cash-flow/t-account?period=&offset=`; Giriş/Çıkış açılır grupları, net bant. Mobilde kapalı özet kartı (Bugün Giriş/Çıkış/Net) → dokununca tam görünüm. **Kart başlığı sadece "Nakit Akım"** (2026-07-06 — "T Hesap Cetveli" ibaresi kullanıcı isteğiyle kaldırıldı; içeride hâlâ T-hesap düzenidir). Mobilde kapalıyken **"detay için dokun ›" ipucu kartın sağ üst köşesinde**. **Bekleyen/Gerçekleşen segmenti + Tarih görünümü (2026-07-06, tasarım "Nakit Akım T-Hesap.dc.html"):** her sütunda iki-satırlı **segment** (✓ Gerçekleşen | Bekleyen — aynı liste yerinde değişir, varsayılan Bekleyen) + sütun başlığındaki **takvim ikonu** ile kategori↔gün gruplaması geçişi. **Gün içi grup sırası (2026-07-07, kullanıcı kararı):** aynı güne denk gelen ödeme grupları sabit öncelikle dizilir — **çek → kredi/leasing → KK borcu → vergi → SGK → [diğerleri: maaş, stopaj, kira…] → fatura (düzenli ödemeler) → cari ödemeleri (en son)**; eşit öncelikte tutar azalan. Tek kaynak: `lib/utils/cashflow.ts` `daySourceRank()` (testli). "İlk açık ödeme" nakit yürüyüşü aynı `dateBuckets` sırasını izlediğinden gün içi nakit de bu önceliğe göre tüketilir (kırmızı satır buna göre kayar). Gelir yeşil (emerald), gider altın (brass). (Finansman rozeti + Faaliyet/Finansman neti çipleri 2026-07-06 gösterimden kaldırıldı; backend verisi durur.) Kolon toplamları/Net değişmez (detay: `backend/app/routers/finance/CLAUDE.md` T-Hesap bölümü).
   - **Nakit akışı grafiği + Vadesi Geçenler kartın İÇİNDE (2026-07-06, "Nakit Koruma" bileşeni kaldırıldı):** Başlığın hemen altında **`RunwayChart`**, kartın en altında **`OverdueList`**.
     - **`RunwayChart` — DÖNEM-DUYARLI bankadaki-nakit runway (mutlak bakiye) eğrisi:** **"BANKADAKİ NAKİT" başlığı** = SAF banka nakdi (`startEur` prop = `runwayStore.data.start_eur` = `_compute_start_eur`; **Bankalar KPI ile TEK kaynak**, C2 — 2026-07-16'da başlık `total_balance_eur`'den buna çekildi: `total_balance_eur` bugün son ekstreden sonraysa bugünün ÖDENMEMİŞ ödemesini düşüyordu → "para hâlâ bankada" ilkesine aykırı understatement). **EĞRİ** ise **`eur_balances.daily`**'den gelir (`cashFlowCache.eurBalances`; **Nakit Akım sayfasıyla AYNI kaynak** → iki görünüm tutarlı) — GERÇEK günlük banka bakiyesi (geçmiş=fiili banka hareketlerinden, gelecek=projeksiyon; eğrinin bugünkü noktası bugünün planlı ödemelerini yansıttığından başlığın ALTINDA seyredebilir — bu bilgilendiricidir, çelişki değil). Seçili dönemin `start_date..end_date` aralığındaki günler dilimlenip çizilir → **dönem sekmesi ve ileri/geri gezinmeyle değişir**. Bakiye 0 altına düşerse **negatif uyarısı** + en düşük bakiye + dönem sonu. **NOT (2026-07-06):** önce T-Hesap kümülatif akışından *geriye* hesaplanıyordu → geçmiş bakiyeler yanlış çıkıyordu (1 Tem gerçek €6.822 iken −€14.916 gösteriyordu); `eur_balances` gerçek bakiyesine geçildi (T-Hesap `curve`/`start_balance` eklemeleri geri alındı). **Bayat cache düzeltmesi (2026-07-07):** başka sayfadayken (ör. Bankalar'da ekstre yükleme) gelen `finance_updated` event'i kaybolup Panel'e dönüşte grafik eski bakiyeyi çiziyordu (mount "cache doluysa çekme" diyordu, F5 gerekiyordu) — artık `cashflow.svelte.ts` store'u event'te tazelik damgalarını sıfırlar, mount `isEurBalancesStale()` ile boş-veya-bayatsa yeniden çeker (detay: `docs/modules/nakit-akim.md` State Yönetimi).
     - **`OverdueList` — vadesi geçmiş ödenmemiş kalemler** (kartın en altında), kaynak türüne göre gruplu; `finance.cash_flow` use → tarih seçerek KALICI öteleme (`POST /cash-flow/defer-batch`). Paylaşımlı **`lib/stores/cashflow/runway.svelte.ts`** deposundan (`GET /finance/cash-flow/runway`) beslenir. İKİ bölüm: **"Vadesi Geçenler"** (kırmızı, GİDER — `overdue`, ötelenebilir) + **"Vadesi Geçen Tahsilatlar"** (amber/+yeşil, GELİR — `overdue_income`; beklenen ama gelmemiş avans/kira, salt gösterim, ötelemesiz — kullanıcı isteği 2026-07-07). Açılan grup detayında gün alt-başlıkları altında kalemler; **cari** ("Cari Ödemeleri") grubunda aynı firmaya aynı gün birden çok ödeme TEK toplu satırda birleşir (`memberRows` + `AGGREGATE_LABELS`, T-Hesap ile aynı kural; kredi/çek ayrı kalır — 2026-07-07). Dönemden bağımsız "şu an geçmiş olanlar" → dönem sekmesiyle değişmez.
     - **İlk açık ödemesi kırmızı (2026-07-07; çift-sayım düzeltmesi 2026-07-16):** Nakit Çıkış **tarih görünümü + bekleyen segmentinde**, bugünkü **SAF** banka nakdinden (`runwayStore.data.start_eur` = backend `_compute_start_eur`) başlayıp bekleyen ödemeleri kronolojik yürüten `tippingCikis` derived'ı, bakiyeyi İLK kez negatife düşüren **tek satırı** kırmızıya boyar + "⚠ nakit buraya yetmiyor" rozeti (kullanıcı: "sadece paranın yetmediği ilk ödeme"). Gün içi giriş o günün ödemelerinden önce eklenir; yürüyüş render ile AYNI `dateBuckets` üzerinden gider → boyanan satır gösterilen sırayla birebir eşleşir. **KRİTİK — başlangıç `total_balance_eur` DEĞİL `start_eur`:** eski sürüm `eur_balances.total_balance_eur` kullanıyordu; o değer bugün son banka ekstresinden sonraysa bugünün ÖDENMEMİŞ ödemesini ZATEN düşüyordu → yürüyüş aynı ödemeyi tekrar düşünce ÇİFT sayım olup ödeme henüz ödenmemişken erken "yetmiyor" damgası basıyordu (kullanıcı bulgusu 2026-07-16: "bugünkü ödeme henüz ödenmemişken neden hem nakitten düşüyor hem de yetmiyor diyor"). Saf nakitten başlayınca her ödeme tam bir kez düşülür. Yürüyüş mantığı saf `cashflow.ts::firstTippingRow`'da (testli). İlk-negatif gün grafiğin (`eur_balances` eğrisi) negatif günüyle tutarlı (eğri = son-of-gün projeksiyonu; saf nakitten bugünün ödemesi düşülünce eğrinin bugünkü noktasına eşitlenir).
