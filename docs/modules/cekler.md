...
### Faz 1 (2026-07-11) — iki-eşikli öneri, çapraz-para, 1-N ve geri alma

- **İki-eşikli bant:** skor ≥ 20 otomatik eşleşir; **8–19 bandındaki** en iyi aday
  "Eşleşme Önerileri" kuyruğuna düşer (`event_matches method='suggestion'`; Nakit Akım
  sayfasındaki panelden Onayla/Reddet).
- **Çapraz-para önerisi:** birebir eşleşemeyen döviz çek için defter kuruyla (işlem günü)
  beklenen TL ±%1 bandındaki TL gideri **yalnız öneri** olur (skor 40) — otomatik uygulanmaz.
- **1-N:** aynı cariye ait vadesi ±2 gün kümelenen TL çeklerin toplamı bir banka giderine
  denk gelirse otomatik topluca kapanır; manuel karşılığı `POST /cash-flow/match-checks-batch`
  (≤20 çek, toplam ±0.02 doğrulamalı).
- **Geri alma:** `POST /cash-flow/unmatch-check` yanlış eşleşen çeki `pending`'e döndürür
  (banka hareketi serbest kalır, `event_matches` izi silinir).
- Eşleşme kurulumu artık ortak `apply_check_bank_match` uygulayıcısıyla yapılır
  (FOR UPDATE SKIP LOCKED yarış-korumalı). Detay: `docs/modules/nakit-akim.md` "Faz 1".
...
## Eşleşmeyi Geri Al (2026-07-11, frontend)

Banka ile eşleşmiş (`bank_transaction_id` dolu + `status='paid'`) çekin eşleşme rozetinin
yanında ikon-only **Geri Al** butonu (Lucide `Undo2`, `canUse` guard) bulunur — hem masaüstü
tablo hem mobil kart görünümünde (`matchBadge` snippet'i içinde, yoğun-tablo ikon-only
istisnası). `ConfirmDialog` onayı ("çek yeniden Bekliyor durumuna dönecek") sonrası
`POST /api/finance/cash-flow/unmatch-check {check_id}` çağrılır: çek **Bekliyor**'a döner,
banka hareketi serbest kalır (yanlış otomatik eşleşmenin ucuz düzeltmesi). Başarıda toast +
`loadAll()`. Endpoint izni `finance.cash_flow use` — backend detayı:
`docs/modules/nakit-akim.md` "Geri alma uçları (#10)".
