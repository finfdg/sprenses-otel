...
- `GET /api/finance/cash-flow/chart` — **Nakit akım grafiği serisi (2026-08-19)** (view; `period=daily|weekly|monthly|yearly` + `back`/`forward`/`offset`, tavan 60 kova; her kova `income_*`/`expense_*` [realized/planned/overdue/held/info], `income_total`/`expense_total`/`net_eur`, kalem sayaçları; **tek sayı kuralı:** aynı period/offset için t-account toplamlarıyla birebir — dönüşüm/gruplama kuralları `t_account`'tan import edilir; vadesi geçenler GÖRÜNÜR ama toplam/net dışı; yanıtta `accounts` anlık banka bakiyeleri [`_helpers.bank_snapshot`, IBAN yalnız son 4], `total_balance_eur`, `overdue_*_eur`, `skipped_no_rate`; `chart_limiter` 30/dk)
...
- `GET /api/finance/cariler/monthly-balances?year&month&mode=fifo|period&hide_zero` — **Aylık Bakiye sekmesi** (2026-07-23): `fifo` = ay sonuna kadar kesilmiş TÜM faturaların bugüne dek ödenmeyen FIFO kalanı (devreden dahil; invoiced/closed/remaining); `period` = ay sonu yürüyen bakiye. Sayfalama yok (≈cari sayısı satır), sıralama frontend'de. view yeterli, onaydan muaf
...
- `GET /api/accounting/mutabakat/items/pdf` — Uyuşmazlık listesi **PDF raporu** (2026-07-22): `/items` ile aynı filtre+sıralama parametreleri, sayfalama yok (1500 satır tavanı — aşım raporda açıkça belirtilir); yatay A4, Türkçe font (₺ dahil), `heavy_limiter`. Salt-okuma GET → onaydan muaf. accounting.mutabakat view
...
### Finans — Bankalar
- `GET /api/finance/banks/accounts/` — Banka hesap listesi. **Hareket filtresi (2026-08-31):** `?date_from&date_to&min_amount&max_amount` (tutar mutlak değer, hesabın kendi para birimi) — filtre varsa yalnız koşula uyan hareketi olan hesaplar döner, `transaction_count` filtreli sayıdır; ters aralık 400
...
- `GET /api/finance/banks/accounts/{id}/transactions` — Hesap işlemleri (yanıt `source` alanı döner: statement/manual); hesap listesiyle aynı hareket filtreleri `date_from`/`date_to`/`min_amount`/`max_amount` (2026-08-31)
...
### Satış — Acente Finansal Takip (yıllık ay × acente rapor, `/dashboard/satis/acente-finans`)
- `GET /api/sales/acente-finans/?year=YYYY` — Seçili yıl için grup × ay matrisi (EUR): alınan/mahsup avans (Sedna 340 hareket snapshot'ı `sales_advance_transactions`), haricen tahsilat (120 alacak; `VİRMAN` bacağı hariç), rezervasyon ciro/adet (çıkış ayı), kesilen fatura, açık/vadesi geçen hak ediş (`sales_invoices` FIFO kalan + `receivable_terms`; kullanılmamış 340 avansı önce buna FIFO mahsup) ve ileri hak ediş tahmini (`projected_*`); `month_end_receivable = open_due + projected_due`; grup satırında `members` kırılımı. **Kur (2026-09-01):** akışlar hareket tarihindeki TCMB kuruyla (`utils/fx_rates.RateBook`, USD/GBP çapraz), açık alacak/avans havuzu bugünkü kurla; kur bulunamayan hareket `source_counts.skipped_no_rate`. `sales.acente_mahsup` view, GET-only (onay dışı), cache yok. Detay: `docs/modules/acente-finans.md`
