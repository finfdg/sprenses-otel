...
ayrı bir projeksiyondur; o gün DEĞİŞMEDİ (bugünkü ödemeyi projeksiyona yansıtmaya devam etti).
**GÜNCELLEME (2026-07-18, kullanıcı kararı):** eğri de hizalandı — aşağıdaki bölüme bkz.
...
## Bakiye Eğrisi Bugün Noktası = Saf Banka Nakdi (2026-07-18)

**Kullanıcı isteği:** "Henüz ödeme yapılmadı, ödenip ödenmeyeceği de belli değil — grafikteki bugün
nokta bakiyesini de bankalara eşitle." (Canlı: başlık €95.664, eğri bugün noktası €91.070; fark bugün
vadeli ödenmemiş ₺247.199 düzenli ödemenin projeksiyondan düşülmesiydi.)

**Değişiklik (`eur_balances.py` kümülatif bakiye bloğu):** son-ekstre-sonrası günlerde projeksiyona
yalnız **`dt > bugün`** planlı kalemler girer (gider VE beklenen gelir simetrik). Bugün ve öncesi
vadeli ödenmemiş kalem bakiyeden düşülmez → **eğrinin bugün noktası = `start_eur` = Bankalar KPI**
(2026-07-06 "para hâlâ bankada" ilkesinin bugüne genişletilmesi; 2026-07-16 liste sınırı `<= bugün`
ile de tutarlı — kalem listede Vadesi Geçenler'de, eğride düşülmemiş). `total_balance_eur` (Nakit
Akım sayfa başlığı) aynı kaynaktan → o da saf banka nakdi. Günlük/aylık gider-gelir toplamları
DEĞİŞMEDİ. Test: `test_payment_deferral.py::TestEurBalanceOverdueVendor::
test_today_due_unpaid_not_subtracted_from_today_point`. Detay: `docs/modules/nakit-akim.md`.
