"""EUR bazlı bakiye hesaplayıcı — `cash_flow/eur_balances.py`'den BİREBİR çıkarım (2026-09-02).

Yeniden yapılandırma (katman yönü: router → service → model). `compute_eur_balances` fonksiyonu
(480 satır; iç kapanışları `_is_held_future` / `get_eur` / `get_usd` / `get_gbp` / `to_eur` ile
birlikte, İÇ BÖLÜNME YAPILMADAN) `app/routers/finance/cash_flow/eur_balances.py`'den satırı satırına,
DEĞİŞTİRİLMEDEN taşındı (finansal parmak izi kapısı: eski-kod/yeni-kod 41 değişmez sıfır fark
vermelidir — hiçbir varsayılan (`date_cls.today()`), yuvarlama, guard, kategori-adı listesi ya da
sorgu sırası değiştirilmedi). Fonksiyon içi lazy import'lar (hold_service / deferral_service /
cc_projection_service / contract_projection_service) monkeypatch hedefi + döngü kırıcı olduğundan
lazy bırakıldı. `MIN_DATE` eskisi gibi `app.utils.finance_helpers`'tan gelir (servis → utils yönü).
`eur_balances.py` router'ı `compute_eur_balances`'ı geriye uyumluluk için yeniden dışa verir
(testler — test_eur_balances_seed / test_payment_deferral / test_cash_flow_hold /
test_cash_flow_matching — ve `report.py` o yoldan çözer); endpoint ince sarmalayıcıdır
(limiter + `compute_eur_balances`).

Özgün açıklama (eur_balances.py):
