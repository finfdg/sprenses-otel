"""Cari analitik görünümleri — Aylık Bakiye (FIFO Kalan / Dönem Sonu) ve Yıllık Ciro (HTTP'siz servis).

**Birebir taşıma (2026-09-02, yeniden yapılandırma — fingerprint-kapılı):** Bu modül
`app/routers/finance/cariler/analytics.py` router'ından `_DEVIR_MATCH` / `_MIN_AMOUNT`
sabitlerinin, `_vendor_map` yardımcısının ve `get_monthly_balances` / `get_yearly_turnover`
endpoint GÖVDELERİNİN (`monthly_balances(db, year, month, mode, hide_zero)` /
`yearly_turnover(db, year)` — kwarg adları endpoint query parametreleriyle aynı) hiçbir ifade
değiştirilmeden kesilip alınmasıyla oluştu. Sorgu/filtre sırası, yuvarlama ve eşikler aynen
korunur — finansal parmak izi (41 değişmez) eski-kod/yeni-kod karşılaştırmasında SIFIR fark
vermelidir. Router (`analytics.py`) yalnız `router` + iki ince sarmalayıcı endpoint'i tutar ve
buradaki TÜM adları geriye uyumluluk için modül düzeyinde yeniden dışa verir
(`services/audit_finance_invariants.py` endpoint fonksiyonlarını o yoldan çağırır).

Özgün açıklama (analytics.py): 2026-07-23 "Cariler modülü yeniden tasarımı" ile eklendi.
Salt-okuma GET'ler — onaydan muaf, finance.cariler view yeterli. Satır sayısı cari sayısıyla
sınırlı (≈300) olduğundan sayfalama yoktur; sıralama frontend'de yapılır.
"""
...
def monthly_balances(db: Session, year: int, month: int, mode: str, hide_zero: bool):
...
def yearly_turnover(db: Session, year: int):
