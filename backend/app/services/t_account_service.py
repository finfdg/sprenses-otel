"""Nakit Akım T Hesap Cetveli — hesaplama çekirdeği (HTTP'siz domain servisi).

**Birebir taşıma (2026-09-02, yeniden yapılandırma Faz 3 — fingerprint-kapılı):** Bu modül
`app/routers/finance/cash_flow/t_account.py` router'ından sabitlerin, alt-çizgili yardımcıların
ve `t_account` endpoint gövdesinin (hız-sınırı kontrolü SONRASI) **hiçbir ifade değiştirilmeden**
kesilip alınmasıyla oluştu. Parametre adları, ifade sırası, `date_cls.today()` çağrıları,
yuvarlama ve guard'lar aynen korunur — finansal parmak izi (41 değişmez) eski-kod/yeni-kod
karşılaştırmasında SIFIR fark vermelidir.

Router (`t_account.py`) yalnız `router` + `taccount_limiter` + endpoint'i tutar ve buradaki
TÜM adları geriye uyumluluk için modül düzeyinde yeniden ihraç eder (testler, `chart.py` ve
`services/audit_finance_invariants.py` o yoldan import eder / monkeypatch hedefler).

Gövde-içi lazy import'lar (`hold_service.get_hold_set`, `cc_projection_service.
due_reserve_projections`, `contract_projection_service.contract_inflow_projections`)
BİLEREK fonksiyon içinde kalır — testler bu modül-attribute'larını monkeypatch'ler
(`tests/test_cash_flow_taccount.py`), modül düzeyine çekilirse patch'ler etkisiz kalır.

Orijinal router docstring'i (iş kuralı özeti):
...
from app.services.fx_rates import CROSS_EUR_CURRENCIES
...
def compute_t_account(db: Session, period: str, offset: int) -> dict:
    """Dönem bazlı T hesap cetveli — giriş/çıkış grupları, EUR karşılıklarıyla.

    `t_account` endpoint gövdesinin (hız-sınırı kontrolü sonrası) birebir kopyası;
    parametre adları endpoint'in query param'larıyla aynı (`period`, `offset`).
    """
