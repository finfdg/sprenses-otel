"""Nakit Koruma / Runway hesaplayıcı — `cash_flow/runway.py`'den BİREBİR çıkarım (2026-09-02).

Yeniden yapılandırma (katman yönü: router → service → model). Sabitler (`SOURCE_LABELS`,
`TR_MONTHS`, `TRANSFER_CATEGORIES` — runway'in KENDİ kopyaları, `t_account`'ınkilerle bilerek
ayrı), `_compute_start_eur`, `_event_eur` (RUNWAY varyantı: `<= 1.0` guard'ı + iki cache — `t_account.
_event_eur`'den FARKLI, birleştirilmedi), `_item_name`, `_natural_date` ve `runway` endpoint'inin
gövdesi (`compute_runway`) `app/routers/finance/cash_flow/runway.py`'den satırı satırına,
DEĞİŞTİRİLMEDEN taşındı (finansal parmak izi kapısı: eski-kod/yeni-kod 41 değişmez sıfır fark
vermelidir — hiçbir varsayılan, yuvarlama, guard ya da sorgu sırası değiştirilmedi). Fonksiyon içi
lazy import'lar (dividend_service / hold_service / cc_projection_service /
contract_projection_service) monkeypatch hedefi + döngü kırıcı olduğundan lazy bırakıldı.
`runway.py` router'ı tüm bu adları geriye uyumluluk için yeniden dışa verir (testler ve
`audit_finance_invariants` o yoldan çözer); endpoint ince sarmalayıcıdır (limiter + `compute_runway`).

Özgün açıklama (runway.py):
...
from app.services.bank_snapshot_service import CROSS_EUR_CURRENCIES, _get_eur_rate, _get_fx_buying, bank_snapshot
...
# Public alias — dış tüketiciler için (gövde aynı nesne; sarmalayıcı değil).
compute_start_eur = _compute_start_eur
...
def compute_runway(db: Session) -> dict:
    """Nakit koruma / runway — içinde bulunulan ay için EUR nakit projeksiyonu.

    `runway` endpoint gövdesinin BİREBİR taşınmış hali (2026-09-02); endpoint yalnız rate-limit
    kontrolü yapıp bunu çağırır ve sonucu değiştirmeden döner.
    """
