...
<div class="max-w-6xl mx-auto mt-2 md:mt-4 px-3 sm:px-4">
	...
	<div class="mb-5 md:mb-6">
		...
		<p class="text-sm text-gray-400 mt-1">İşte bugünkü genel bakış</p>
	</div>
	...
	{:else}
		...
		<div class="flex flex-wrap gap-3 md:gap-4 mb-5 md:mb-6">
			...
			{#if canBanks}
				<a href="/dashboard/finans/bankalar" class="group bg-white rounded-xl border border-gray-100 hover:border-teal-200 hover:shadow-md p-4 md:p-5 transition-all flex-1 min-w-[140px]">
					<div class="flex items-center gap-2 mb-2">
						...
						<span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Bankalar</span>
					</div>
					<div class="text-lg md:text-xl font-bold text-gray-800">
						{bankTotal || '—'}
					</div>
					<p class="text-xs text-gray-400 mt-1">{bankSublabel}</p>
				</a>
			{/if}
			...
			{#if canChecks}
				<a href="/dashboard/finans/cekler" class="group bg-white rounded-xl border border-gray-100 hover:border-amber-200 hover:shadow-md p-4 md:p-5 transition-all flex-1 min-w-[140px]">
					<div class="flex items-center gap-2 mb-2">
						...
						<span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Çekler</span>
					</div>
					<div class="text-lg md:text-xl font-bold text-gray-800">
						{checksAmount || '—'}
					</div>
					<p class="text-xs text-gray-400 mt-1">{checksSublabel}</p>
					{#if checksDetail}
			...
			<!-- Krediler -->
			{#if canCredits}
				<a href="/dashboard/finans/krediler" class="group bg-white rounded-xl border border-gray-100 hover:border-yellow-200 hover:shadow-md p-4 md:p-5 transition-all flex-1 min-w-[140px]">
					<div class="flex items-center gap-2 mb-2">
						<div class="w-8 h-8 rounded-lg bg-yellow-50 flex items-center justify-center">
							<svg class="w-4 h-4 text-yellow-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
								<path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
							</svg>
						</div>
						<span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Krediler</span>
					</div>
					<div class="text-lg md:text-xl font-bold text-gray-800">
						{creditAmount || '—'}
					</div>
					<p class="text-xs text-gray-400 mt-1">{creditSublabel}</p>
					{#if creditDetail}
						<p class="text-xs text-yellow-600 mt-0.5">{creditDetail}</p>
					{/if}
				</a>
			{/if}
			...
			{#if canAdvances}
				<a href="/dashboard/finans/avanslar" class="group bg-white rounded-xl border border-gray-100 hover:border-purple-200 hover:shadow-md p-4 md:p-5 transition-all flex-1 min-w-[140px]">
					<div class="flex items-center gap-2 mb-2">
						...
						<span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Avanslar</span>
					</div>
					<div class="text-lg md:text-xl font-bold text-gray-800">
						{advancesAmount || '—'}
					</div>
					<p class="text-xs text-gray-400 mt-1">{advancesSublabel}</p>
					{#if advancesDetail}
			...
			{#if canCariler}
				<a href="/dashboard/finans/cariler" class="group bg-white rounded-xl border border-gray-100 hover:border-red-200 hover:shadow-md p-4 md:p-5 transition-all flex-1 min-w-[140px]">
					<div class="flex items-center gap-2 mb-2">
						...
						<span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Cariler</span>
					</div>
					<div class="text-lg md:text-xl font-bold text-gray-800">
						{vendorAmount || '—'}
					</div>
					<p class="text-xs text-gray-400 mt-1">{vendorSublabel}</p>
				</a>
			{/if}
		</div>
