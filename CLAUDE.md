...
| Denetim otomasyonu | — | `sprenses-denetim-auto.timer` (2026-07-25; **2 saatte bir :40** — 00:40/02:40/…/22:40 Europe/Istanbul, günde 12 koşu. Kilit yüzünden çakışan tetikleme **atlanır, kuyruğa alınmaz**. `backend/cron_denetim_auto.py` açık bir denetim bulgusunu **izole git worktree'de** headless `claude -p` ile düzelttirir, testleri koşar, yeşilse master'a merge + deploy eder ve `/api/health` 200 dönmezse **otomatik geri alır**. **FİNANSAL KAPI:** deploy öncesi aynı DB üzerinde eski-kod/yeni-kod parmak izi karşılaştırılır (41 değişmez: T-Hesap, runway, FIFO, kredi/çek, satış, kur; salt-okunur transaction). Bir sayı oynuyorsa deploy EDİLMEZ. A/A2 kontrol ölçümü canlı veri sürüklenmesini ayrıştırır. **Pencereli değişmezler yetmez** — en az bir penceresiz tam tarama (`fe_event_eur_tam_tarama`) korunmalı; 2026-07-25'te FIN-001 kasten geri alındığında pencereli ölçümler onu kaçırdı. **Migration / bu script / systemd birimi / finansal kapının kendisi otomatik deploy EDİLMEZ** — inceleme kuyruğunda bekler. Acil durdurma anahtarı DB'de (`audit_automation_config.enabled`, UI'dan yönetilir) — timer'ı durdurmaya gerek yok. `--setting-sources user` ile proje Stop hook'u devre dışı bırakılır (canlı çalışma ağacını commit'lemesin); bunun bedeli PreToolUse gizli-dosya bekçisinin de kapanmasıdır → script kendi `_looks_secret()` bekçisini uygular. Unit'ler `scripts/systemd/`'de git'te VAR. Detay: `docs/modules/denetim-takip.md`) |
...
- **CI (yapılandırıldı, HENÜZ ETKİN DEĞİL — 2026-07-25):** `.github/workflows/ci.yml`, push/PR'da üç iş çalıştıracak şekilde **yapılandırılmıştır**: **lint & tip** (backend `ruff check --select E9,F63,F7,F82` kritik hata kapısı + frontend `svelte-kit sync` → `svelte-check`), **backend** (pytest, Postgres service container + `tests/ci/` bootstrap ile sıfırdan test DB) ve **frontend** (vitest). Lint/tip kapısı hata durumunda job'ı KIRMIZI yapar (QUAL-001, v4 denetim Boyut 2 — regresyon: `backend/tests/test_ci_lint_gate.py`). Ruff bloklayıcı kümesi bilerek dar (gerçek-hata sınıfları, bugün 0 ihlal); tam kural seti `backend/ruff.toml`'da yerel-danışma olarak açık, temizlendikçe bloklayıcı listeye taşınır. **Depo ayarı 2026-07-26'da düzeltildi** (CICD-010): `actions/permissions → enabled:true`, `ci.yml` artık her push'ta **gerçekten tetikleniyor** (0 koşu → ilk koşu `30187332124`). **ANCAK CI hâlâ fiilen çalışmıyor:** GitHub hesabı **faturalandırma kilidinde**, işler başlamadan `failure` oluyor (*"The job was not started because your account is locked due to a billing issue"*) — depo public olduğu için Actions dakikaları ücretsiz olmasına rağmen. Bu **hesap düzeyinde** bir engel (yeni alt-bulgu **CICD-010b**), yalnız hesap sahibi çözebilir: GitHub → Settings → Billing. Kilit çözülünce bu ifade "her push/PR'da çalışır" olarak güncellenmeli. Regresyon bekçisi: `backend/tests/test_ci_cicd010_actions_enabled.py` (Actions tekrar kapatılırsa / push tetikleyicisi kaldırılırsa / bir iş `if: false` ile susturulursa kırmızı). (Depo hâlâ public → SEC-001 açık — bkz. `docs/denetim/2026-07-25-v4-kurumsal-denetim.md`.)
...
- **DB adı:** sprenses
- **Kullanıcı:** sprenses
- **Tablolar (86):** stock_depots, stock_products, stock_movements, personnel, attendance_logs, attendance_settings, shift_definitions, shift_assignments, users, roles, modules, role_module_permissions, conversations, conversation_members, messages, audit_logs, push_subscriptions, notifications, error_logs, vendors, vendor_uploads, vendor_transactions, vendor_bank_accounts, vendor_notes, sales_invoices, sales_collections, sales_advances, transaction_categories, bank_accounts, bank_statements, bank_transactions, checks, check_uploads, credit_products, credit_payments, credit_card_statements, credit_card_transactions, advances, departments, budgets, budget_categories, finance_events, scheduled_definitions, scheduled_entries, exchange_rates, cash_flows, reservations, reservation_uploads, room_types, agency_groups, approval_workflows, approval_workflow_requestor_roles, approval_workflow_approver_roles, approval_workflow_steps, approval_requests, approval_request_logs, payment_instruction_lists, payment_instruction_items, payment_deferrals, dividend_distributions, dividend_shareholders, dividend_installments, dividend_payments, sedna_recon_runs, sedna_bank_recon, event_matches, fx_differences, finance_period_locks, agency_code_map, agency_code_overrides, agency_contracts, ai_conversations, ai_messages, ai_usage, cash_flow_holds, contract_action_tiers, contract_actions, contract_allotments, contract_deductions, contract_documents, contract_installments, contract_payment_plans, contract_periods, contract_room_types, receivable_terms, sales_advance_transactions
- **Saat dilimi:** Europe/Istanbul (her bağlantıda SET edilir)
- **Migrations:** `cd backend && source venv/bin/activate && alembic upgrade head`
- **Otomatik yedek (2026-06-22, denetim D15-1 — tek Kritik):** Günlük `pg_dump -Fc` → `/var/backups/sprenses-db/` (`scripts/db-backup.sh`, systemd `sprenses-db-backup.timer` 03:00 Istanbul, bütünlük doğrulamalı, 30-yedek rotasyon). Geri yükleme + **restore tatbikatı**: `scripts/db-restore.sh` (argümansız = en son yedeği geçici DB'ye yükle/say/sil; `<dump> sprenses` = üretime, `EVET` onaylı). **Kod yedeği (git/Stop hook) ≠ DB yedeği** — bu ayrı. **Off-site (S3, denetim DR-002 — CANLI 2026-07-25):** `s3://sprenses-dr-2026/sprenses` — bucket **eu-west-1**, sunucu eu-north-1 (**farklı bölge zorunlu**, `_offsite-lib.sh:offsite_assert_cross_region`; bölge okunamıyorsa **reddeder**, varsaymaz). Günlük DB dump + uploads aynası (`aws s3 sync`, `--delete` YOK) otomatik gider; EC2 `SprensesBackupRole` IAM rolüyle çalışır (sunucuda uzun ömürlü AWS anahtarı **yok**). Off-site hatası **işi başarısız yapar** → OnFailure alarmı. Görünürlük: `/var/backups/sprenses-db/backup-state.json` (**tek kaynak**) → `GET /api/system/backup/data-status` + Sistem▸Yedekleme "Veri Yedeği" paneli + `health-thresholds.py` off-site eşiği. Tatbikat: `scripts/db-restore.sh --offsite` (indir→bütünlük→geçici DB→satır say→S3'ten 5 belge checksum). Kurulum/yeniden kurulum: `scripts/provision-offsite-backup.sh <bucket> <farklı-bölge>` (idempotent). Test: `tests/test_backup_offsite.py` (19 test, sahte `aws` CLI). **Tuzak:** IAM'de `s3:GetBucketLocation` **koşulsuz** ifadede olmalı — `s3:prefix` koşuluna konursa AccessDenied verir ve bölge bekçisi körleşir (canlıda yaşandı). Detay: `docs/modules/yedekleme.md`, tatbikat: `docs/denetim/2026-07-25-offsite-tatbikati.md`.
...
- Muhasebe (accounting) → Vergiler (accounting.taxes), Düzenli Ödemeler (accounting.recurring), Alınan Kiralar (accounting.rent_income), Verilen Kiralar (accounting.rent_expense), Temettü (accounting.dividend), Kullanıcı Fiş İcmali (accounting.fis_icmali), Mizan (accounting.mizan), Sedna Mutabakat (accounting.mutabakat)
...
- Sistem (system) → Kullanıcılar (system.users), Roller (system.roles), Modüller (system.modules), Audit Loglar (system.audit_logs), Hata Logları (system.error_logs), Onay Akışı (system.approval), Sunucu (system.server), Yedekleme (system.backup), Dokümanlar (system.docs), Denetim Takip (system.denetim — **2026-07-25**: kurumsal denetim raporunu yaşayan veriye çevirir; 23 boyutluk canlı skor panosu + bulgu tablosu + her bulgu için kopyalanabilir Claude Code komutu + 5 saatlik otomatik düzeltme koşusu; migration `c4d8e2f6a1b9`; detay `docs/modules/denetim-takip.md`)
- Satış (sales) → Acente Mahsup & Nakit Akım (sales.acente_mahsup — **BİRLEŞİK, 2026-07-09**: Rezervasyonlar + Günlük Hareketler + Oda Tipleri + projeksiyon sekmeleri tek modül/sayfada; eski sales.hotel_reservation / sales.daily_reservations / sales.room_types modülleri kaldırıldı, migration `b3c9d5e7f1a2`), Kontratlar (sales.kontratlar — **2026-07-17**: 16 tur operatörünün kontrat arşivi + ödeme planı/taksit + EB/SPO aksiyon + kontenjan + kesinti; acente-mahsup sayfasında AYRI izinli sekme; migration `b7d2f4a8c1e6`; detay `docs/modules/kontratlar.md`), Acente Finansal Takip (`/dashboard/satis/acente-finans` — **2026-08-05**: aynı `sales.acente_mahsup` izniyle yıllık ay × acente avans/tahsilat/ciro/hak ediş raporu, GET-only; Sedna 340 hareket snapshot'ı `sales_advance_transactions`, migration `e6a1c4f8b2d7`; detay `docs/modules/acente-finans.md`)
